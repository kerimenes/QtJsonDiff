	...
	QWidget(parent)
{
	...
	extractModeEnable = new QCheckBox(common_groupbox);
	extractModeEnable->setText("Extract Mode");
	qjsoncontainer_layout->addWidget(extractModeEnable, 1, 1);
...
void QJsonDiff::on_lefttreeview_clicked(const QModelIndex&)
{
	...
	if(idx.isValid())
	{
		...
		if (extractModeEnable->isChecked()) {
			QJsonTreeItem *myRootItem;
			myRootItem = item;
			QJsonObject lastObj;
			while(myRootItem->key() != "root") {
				bool isArray = false;
				int arrayIndex;
				if (myRootItem->parent()->value().isEmpty() && myRootItem->parent()->type() == QJsonValue::Type::Array) {
					QJsonObject obj = lastObj;
					bool isNumeric = false;
					foreach (QString key, obj.keys()) {
						key.toInt(&isNumeric);
						if (isNumeric) {
							arrayIndex = key.toInt();
							isArray = true;
						}
						else continue;
					}
				}
				if (myRootItem->value().isEmpty() && !lastObj.contains(myRootItem->key()) ) {
					QJsonObject oldObject = lastObj;
					qDebug() << "This is root";
					foreach (QString key, oldObject.keys()) {
						lastObj.remove(key);
					}
					if (isArray) {
						QJsonArray array;
						array.insert(arrayIndex, oldObject);
						lastObj.insert(myRootItem->key(), oldObject);
						continue;
					}
					lastObj.insert(myRootItem->key(), oldObject);
					qDebug() << "counting" << lastObj;
				} else {
					qDebug() << "standart" << lastObj;
					QJsonObject obj;
					obj.insert(myRootItem->key(), myRootItem->value());
					lastObj = obj;
				}
				myRootItem = myRootItem->parent();
			}
			qDebug() << "Last Obj" << lastObj;

			saveExtractFile("extract.json", lastObj);
			if (item->parent()) {
				QJsonTreeItem *upItem = item->parent();
				if (upItem->parent()) {
					QJsonTreeItem *moreUpItem = upItem->parent();
					qDebug() << moreUpItem->key() << "type " << moreUpItem->type();
					if (moreUpItem->parent()) {
						qDebug() << "founded more up item " << moreUpItem->parent()->key();
					} else qDebug() << "not found more more up item";
				}
			}
		}
...
void QJsonDiff::openJson(const QString &filename)
{
	QFile f(filename);
...
void QJsonDiff::saveExtractFile (const QString &filename, const QJsonObject &obj)
{
	openJson(filename);
	qDebug() << "File obj" << fileObj;

	foreach (QString key, obj.keys()) {
		qDebug() << key << "keys";
		QJsonObject tmpObj = obj.value(key).toObject();
	}
	QFile f(filename);
